    ...
    while ($incident=mysql_fetch_array($incident_result))
    {
        ...
        if ($newSlaTime!=-1)
        {
            ...
            if ($incident['slaemail'] < 2)
            {
                ...
                {
                    send_template_email('NEARING_SLA',$incident['id'],$tag,$times['next_sla_time']-$newSlaTime);
                    $emailSent=1; // First email sent
                }

                if ($newSlaTime > $times['next_sla_time'])
                {
                    send_template_email('OUT_OF_SLA',$incident['id'],$tag,$times['next_sla_time']-$newSlaTime);
                    $emailSent=2; // Second email sent
                }
            }
            ...
            if ($emailSent >= 1)
            {
                $sql="UPDATE incidents SET slaemail='{$emailSent}' WHERE id='{$incident['id']}'";
                mysql_query($sql);
