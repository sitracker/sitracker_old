...
function draw_chart($mode, $year, $month='', $day='', $groupid='', $userid='')
{
    ...
    else
    {
        if ($numgroups < 1)
        {
            $html .= "<p class='info'>Nothing to display</p>";
        }
        else
        {
            $html .= "<p class='info'>Nothing to display, check user group membership.</p>";
        }
    }
    $html .= "</table>\n\n";
    ...
    $html .= "<tr><td></td><td class='urgent'>{$strDeclined}</td>";
    $html .= "<td class='review'>{$strNotApproved}</td><td class='idle'>{$strApproved}</td>";
    $html .= "<td class='notice'>{$strApprovedFree}</td></tr>";
    $html .= "</table>\n\n";
...
elseif ($display=='list')
{
    ...
    if (mysql_num_rows($result))
    {
        ...
        if ($approver)
        {
            echo "<p align='center'><a href='{$_SERVER['PHP_SELF']}?display=list&amp;type={$type}&amp;user=all'>{$GLOBALS['strShowAll']}</a></p>";
        }
    }
...
elseif ($display == 'week')
{
    ...
    echo ldate('dS F Y',mktime(0,0,0,$month,$day,$year))." &ndash; ".ldate('dS F Y',mktime(0,0,0,$month,$day+7,$year));
...
else
{
    ...
    if (!empty($selectedday))
    {
        ...
        elseif ($approved == 1)
        {
            ...
            {
                echo "&nbsp;As approver for this holiday you can <a href='add_holiday.php?type=$type&amp;user=$user&amp;year=$selectedyear&amp;month=$selectedmonth&amp;day=$selectedday&amp;length=0'>deselect</a> it.";
            }
        }
        else
        {
    ...
    echo ldate('Y',mktime(0,0,0,$month,$day,$year));
