...
for($year=$startyear;$year<$lastyear;$year++)
{
    ...
    for($month=$startmonth;$month<=$lastmonth;$month++)
    {
        ...
        if ($diff < 0) $diff = "<span style='color: $closedcolour;'>$diff</span>";
        else $diff="<span style='color: $openedcolour;'>$diff</span>";
        echo "<tr><td align=\"center\" colspan=\"$colspan\" style='border-bottom: 2px solid #000;'><p>Total: <b style='color: $openedcolour;'>$monthtotal</b> opened and <b style='color: $closedcolour;'>$monthtotalclosed</b> closed during $monthname $year, difference: <b>$diff</b><br />";
        $diff = ($grandtotal-$grandtotalclosed);
        if ($diff < 0) $diff = "<span style='color: $closedcolour;'>$diff</span>";
        else $diff="<span style='color: $openedcolour;'>$diff</span>";
        echo "Total: <b style='color: $openedcolour;'>$grandtotal</b> opened and <b style='color: $closedcolour;'>$grandtotalclosed</b> closed up to the end of $monthname $year, difference <b>$diff</b></p><br /></td></tr>\n";
    }
    if ($startmonth > 1)
    {
