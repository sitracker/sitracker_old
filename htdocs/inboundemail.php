...
function populate_syslang2()
{
    ...
    $file = "{$CONFIG['application_fspath']}htdocs/i18n/{$CONFIG['default_i18n']}.inc.php";
    if (file_exists($file))
    {
        $fh = fopen($file, "r");
            ...
            if ($error != 1)
            {
                ...
                else
                {
                    ...
                    if ($incident_open == "No")
                    {
                        ...
                        $sql.= "VALUES ('{$updateid}', '0', '{$from_name}', '".mysql_real_escape_string($subject)."', '{$reason}', '$contactid' )";
                        mysql_query($sql);
                        if (mysql_error()) trigger_error(mysql_error(),E_USER_WARNING);
                    }
                    else
